	...
	: GUI_Splitter(_props) constructor
{
	...
	static Layout = function (_force=false) {
		...
		Top.SetProps({
			...
			RealY: ealY,
			RealWidth: ealWidth,
			RealHeight: ottom.Visible ? max(_splitterY - RealY, 0) : RealHeight,
		});
		Top.Layout(_force);

		Bottom.SetProps({
			RealX: ealX,
			RealY: op.Visible ? (_splitterY + Size) : RealY,
			RealWidth: ealWidth,
			RealHeight: op.Visible ? max(RealY + RealHeight - Bottom.RealY, 0) : RealHeight,
		});
